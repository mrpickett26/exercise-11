...
d <- d %>%
    mutate(Migration = case_when(Migration == "3" ~ "3", Migration == "2" ~ "2", Migration ==
        "1" ~ "1"))
...
## Means are significant because none of the CIs cross the vertical line 
library(reshape2)
library(broom)
d <- d %>%
...
interaction.plot(
  ...
  trace.factor = resid_BL_mass2,
  trace.label = "Relative Body Length to Mass",
  fun = base::mean, # make sure we use {base} version
  response = d$Migration,
  ylab = "Mean Migration"
)

#tried to push remote: Permission to difiore/spatial.git denied to mrpickett26.
